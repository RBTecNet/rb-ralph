...
function baseRecord(provider, model, effort) {
  return {
    ...
    inputIncludesCached: null,
    contextCompactionObserved: false,
  };
}

function codex(raw, model, effort) {
  const record = baseRecord("codex", model, effort);
  const visible = [];
  let parsedEvents = 0;
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (/compact/i.test(eventType) || event.context_compaction === true || event.compacted === true) {
      record.contextCompactionObserved = true;
    }
    ...
    if (usage && typeof usage === "object") {
      ...
      record.usageSource = "provider-final-event";
      record.inputIncludesCached = true;
    }
  }
...
function claude(raw, configuredModel, effort) {
  const record = baseRecord("claude", configuredModel, effort);
  let value;
  try {
  ...
  record.measured = record.totalTokens > 0;
  record.usageSource = record.measured ? "provider-result" : "unavailable";
  record.inputIncludesCached = false;
  record.contextCompactionObserved = value.context_compaction === true || value.compacted === true;
...
async function main() {
  const [provider, rawPath, outputPath, model = "unknown", effort = "default"] = process.argv.slice(2);
  if (provider === "placeholder") {
    if (!rawPath) throw new Error("Usage: provider-telemetry.cjs placeholder <usage-json> [provider] [model] [effort]");
    await atomicJson(resolve(rawPath), baseRecord(outputPath || "custom", model, effort));
    return;
  }
  if (!provider || !rawPath || !outputPath || !["codex", "claude"].includes(provider)) {
    throw new Error("Usage: provider-telemetry.cjs codex|claude <raw-output> <usage-json> [model] [effort]");
  }
  const raw = await readFile(resolve(rawPath), "utf8");
  const normalized = provider === "codex" ? codex(raw, model, effort) : claude(raw, model, effort);
