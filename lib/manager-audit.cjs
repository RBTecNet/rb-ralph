...
function scopeTokens(scope) {
  return [...String(scope || "").matchAll(/`([^`]+)`/g)]
    .map((match) => match[1].trim().replace(/^\.\//, "").replace(/\/?\*\*?\/?\*?$/, "").replace(/\/$/, ""))
    .filter(Boolean);
}

function boundaryOwnsTask(boundary, task) {
  const normalized = clean(boundary).replace(/\\/g, "/");
  return scopeTokens(task.scope).some((token) => normalized.includes(token));
}
...
function retrySelection(tasks, report) {
  ...
  const criteriaFromFindings = (report.findings ?? []).flatMap((finding) =>
    String(finding.criteria || "").split(/\s*,\s*/).filter(Boolean));
  const failedRows = (report.rows ?? []).filter((row) => ["FAIL", "UNPROVEN"].includes(row.status)).map((row) => row.id);
  for (const criterion of [...criteriaFromFindings, ...failedRows]) {
  ...
  // A dependency is repeated only when the structured finding names it or its
  // declared scope boundary. Conversational dependency alone is not evidence
  // that already-proven producer code must be rediscovered and re-executed.
  let expanded = true;
  while (expanded) {
    expanded = false;
    for (const taskId of [...selected]) {
      ...
      for (const dependencyId of task?.dependsOn ?? []) {
        ...
        if (!dependency || selected.has(dependencyId)) continue;
        const explicitlyNamed = criteria.has(dependencyId)
          || [...criteria].some((criterion) => expected.get(criterion)?.parent === dependencyId);
        const boundaryNamed = (report.findings ?? []).some((finding) => boundaryOwnsTask(finding.boundary, dependency));
        if (explicitlyNamed || boundaryNamed) { selected.add(dependencyId); expanded = true; }
      }
    }
  }
  return {
...
if (!command || !firstPath) fail("Usage: manager-audit.cjs expected <phase> | validate <phase> <manager-log> | retry-selection <tasks-json> <audit-json> | reconcile <ledger> <audit> <phase> <attempt> <fingerprint> | replay <ledger> <logs> <prompts>");
if (command === "expected") {
...
module.exports = { expectedRows, validate, retrySelection, findingKey };
