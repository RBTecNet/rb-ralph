...
for (const line of validationSource.split(/\r?\n/)) {
  const impactRow = /^impact=(affected|full) reason=(.*)$/.exec(line);
  if (impactRow) {
    impact = { mode: impactRow[1], reason: impactRow[2] };
    continue;
  }
  const command = /^\[([^\]]+)\] command: (.*)$/.exec(line);
  if (command) {
    current = { taskId: command[1], taskIds: command[1].split(","), command: command[2], exitCode: null, durationSeconds: null, reusedFrom: null };
    validations.push(current);
    continue;
  }
  const exit = /^\[([^\]]+)\] exit=(\d+)(?: duration_seconds=(\d+))?(?: reused_from=(.*))?$/.exec(line);
  if (exit) {
...
const result = {
  ...
  provenance: {
    changedPaths: { origin: "orchestrator-generated", trust: "deterministic" },
    executorLog: { origin: "provider-output", trust: "provider-asserted" },
    validationOutput: { origin: /^\[RBT-FINAL\]/m.test(validationSource) ? "operational-verification-output" : "validation-output", trust: "deterministic-process-output" },
  },
