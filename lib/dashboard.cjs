...
async function load(run) {
  ...
  return { ...run, phases, tasks, meta, phaseStatus, phaseAttempt, phaseGates, usage, models, latestEvent, activityLog };
}
...
function recentLogPanel(state, width) {
  ...
  const content = source?.lines?.length ? source.lines : ["Aguardando a primeira saída do provider…"];
...
function render(state) {
  ...
  lines.push(boxLine(`${C.cyan}atividade${C.reset}  ${C.white}${truncate(state.meta.activity || "aguardando", width - 18).trimEnd()}${C.reset}`, width, C.yellow));
  ...
  if (lines.length + 1 + logPanel.length + 1 > maxLines) {
    ...
    const current = Math.max(0, table.findIndex((line) => line.trimStart().startsWith(state.meta.phase || "\0")));
    const room = Math.max(2, maxLines - header.length - logPanel.length - 2);
    const start = Math.max(0, Math.min(current - 1, table.length - room));
    const scrollFooter = truncate(`Mostrando ${room}/${table.length} linhas · ${footer}`, width).trimEnd();
    return [...header, ...table.slice(start, start + room), tableBottom, ...logPanel, `${C.dim}${scrollFooter}${C.reset}`].join("\n");
  }
