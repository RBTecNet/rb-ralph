...
function providerActivity(status) {
  ...
  const counts = `comandos ${status.commandCount ?? 0}, edições ${status.editCount ?? 0}, mensagens ${status.messageCount ?? 0}`;
  const editAge = status.secondsSinceEdit == null ? "sem edição observada" : `última edição há ${status.secondsSinceEdit}s`;
  const warning = Array.isArray(status.warnings) && status.warnings.length ? `; aviso: ${status.warnings.join(", ")}` : "";
  return `${status.role}: ${counts}; ${status.outputBytes} bytes; ${editAge}; última saída há ${status.secondsSinceOutput}s${warning}`;
}
...
async function load(run) {
  ...
  for (const row of await rows(resolve(run.directory, "usage-summary.tsv"))) {
    if (row[0] === "USAGE") usage[row[1]] = row[2];
      ...
      derivedUncachedInputTokens: row[14], contextCompactionCalls: row[15],
    });
  }
  for (const row of await rows(resolve(run.directory, "context-efficiency-summary.tsv"))) {
    if (row[0] === "EFFICIENCY") efficiency[row[1]] = row[2];
    else if (row[0] === "CALL") efficiencyCalls.push({
      callId: row[1], phaseId: row[2], taskId: row[3], attempt: row[4], role: row[5],
      provider: row[6], model: row[7], measured: row[8] === "true", promptBytes: row[9],
      inputTokens: row[10], cachedInputTokens: row[11], uncachedInputTokens: row[12],
      outputTokens: row[13], commands: row[14], edits: row[15], messages: row[16],
      logBytes: row[17], compactionObserved: row[18] === "true", warnings: row[19] || "",
    });
    else if (row[0] === "WARNING") efficiencyWarnings.push({ phaseId: row[1], kind: row[2], count: row[3] });
  }
  ...
  return {
    ...run, phases, tasks, meta, phaseStatus, phaseAttempt, phaseGates, usage, models,
    efficiency, efficiencyCalls, efficiencyWarnings, latestEvent, activityLog, providerStatus,
  };
}
...
function render(state) {
  ...
  const derivedUncachedInput = usage.derivedUncachedInputTokens === undefined
    ? Math.max(0, Number(usage.inputTokens ?? 0) - Number(usage.cachedInputTokens ?? 0))
    : usage.derivedUncachedInputTokens;
  ...
  lines.push(boxLine(`${C.cyan}CHAMADAS${C.reset} ${calls}  ${C.green}medidas ${usage.measuredCalls ?? 0}${C.reset}  ${C.grey}sem métrica ${usage.unmeasuredCalls ?? 0}${C.reset}    ${C.cyan}ENTRADA ACUMULADA${C.reset} ${count(usage.inputTokens)}  cache (parcela) ${count(usage.cachedInputTokens)}  não cache ${count(derivedUncachedInput)}  saída ${count(usage.outputTokens)}`, width, C.magenta));
  lines.push(boxLine(`${C.cyan}PROMPT INICIAL${C.reset} ${count(state.efficiency.promptBytes)} bytes  ${C.grey}~${count(Math.ceil(Number(state.efficiency.promptBytes ?? 0) / 4))} tokens estimados${C.reset}    ${C.cyan}COMPACTAÇÃO${C.reset} ${count(state.efficiency.contextCompactionCalls ?? usage.contextCompactionCalls)} chamada(s)`, width, C.magenta));
  ...
  const measuredEfficiency = state.efficiencyCalls.filter((call) => call.measured);
  const topInput = [...measuredEfficiency].sort((left, right) => Number(right.inputTokens) - Number(left.inputTokens)).slice(0, 3);
  const topCommands = [...state.efficiencyCalls].filter((call) => call.commands !== "unmeasured")
    .sort((left, right) => Number(right.commands) - Number(left.commands)).slice(0, 3);
  const topLog = [...state.efficiencyCalls].sort((left, right) => Number(right.logBytes) - Number(left.logBytes)).slice(0, 3);
  const callLabel = (call) => call ? `${call.phaseId}/${call.taskId} A${call.attempt}` : "sem dados";
  const ranking = (calls, field, suffix = "") => calls.length
    ? calls.map((call) => `${callLabel(call)} ${count(call[field])}${suffix}`).join(" · ") : "sem dados";
  lines.push(boxLine(`${C.cyan}TOP entrada${C.reset} ${ranking(topInput, "inputTokens")}`, width, C.magenta));
  lines.push(boxLine(`${C.cyan}TOP comandos${C.reset} ${ranking(topCommands, "commands")}`, width, C.magenta));
  lines.push(boxLine(`${C.cyan}TOP log${C.reset} ${ranking(topLog, "logBytes", "B")}`, width, C.magenta));
  const localizedRetries = state.efficiencyCalls.filter((call) => call.role === "agent" && call.taskId !== "phase" && Number(call.attempt) > 1).length;
  const repeatedPhases = state.efficiencyCalls.filter((call) => call.role === "agent" && call.taskId === "phase" && Number(call.attempt) > 1).length;
  const efficiencyWarning = state.efficiencyWarnings.length
    ? `  ${C.yellow}⚠ amplificação repetida em ${state.efficiencyWarnings.map((item) => item.phaseId).join(",")}${C.reset}` : "";
  lines.push(boxLine(`${C.cyan}RETRIES${C.reset} localizados ${localizedRetries}  chamadas repetidas de fase ${repeatedPhases}${efficiencyWarning}`, width, C.magenta));
