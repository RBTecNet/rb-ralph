...
const timer = setInterval(() => {
  ...
  if (!timedOut && !firstOutputAt && options.firstOutputTimeout > 0
  ...
  publishStatus(timedOut ? "timeout" : "running", timedOut ? { timeoutKind: timedOut.kind } : {});
  if (!timedOut) return;
  output.write(
...
child.on("close", (code, signal) => {
  ...
  const finalize = () => output.end(() => {
    publishStatus(timedOut ? "timeout" : code === 0 ? "complete" : "failed", {
